...
- `src/lib/rollingMedian.js` — RollingMedian implementation (two heaps)
- `src/lib/random.js` — getRandomPairs (Fisher–Yates)
- `tests/` — Jest unit tests

### Notes & caveats
- `RollingMedian` stores every observation (exact median). Memory grows linearly with observations. For long-running deployments consider approximate summaries (t-digest) or retention strategies.
